...
public class RunUtil {
    ...
    private static final int MESSAGE_RUN_ON_UI_THREAD = 0x1;

    private static Handler sHandler;

    /**
     * execute a runnable on ui thread, then return immediately. see also runOnUiThread()
    ...
    public static void runOnUiThread(Runnable runnable, boolean waitUtilDone) {
        ...
        getHandler().obtainMessage(MESSAGE_RUN_ON_UI_THREAD, pair).sendToTarget();
        if (waitUtilDone) {
            try {
    ...
    private static class InternalHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            if (msg.what == MESSAGE_RUN_ON_UI_THREAD) {
